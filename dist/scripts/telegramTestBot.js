...
function buildLoopEntry(r, adjLabel) {
...
function buildBriefOutput(summary, mode) {
    const lines = [];
...
async function handleBriefWithProgress(token, chatId, filePath, mode) {
    await sendMessage(token, chatId, 'Reading interpreted OpenClaw observations…\nexternalWrite:false');
    let observations;
    try {
    ...
    await sendMessage(token, chatId, `Loaded ${n} interpreted candidate${n !== 1 ? 's' : ''}.\nexternalWrite:false`);
    await sendMessage(token, chatId, 'WorldLoops is adjudicating candidates…\nexternalWrite:false');
    let summary;
    try {
    ...
    await sendMessage(token, chatId, `WorldLoops found ${accepted} open loop${accepted !== 1 ? 's' : ''}, ${context} context item${context !== 1 ? 's' : ''}, ${suppressed} suppressed.\nexternalWrite:false`);
    await sendMessage(token, chatId, buildBriefOutput(summary, mode));
}
async function handleBriefCommand(token, chatId) {
...
async function handleDemoCommand(token, chatId) {
