...
const HIGH_SEVERITIES: MinSeverity[] = ['high', 'critical'];
...
function main(): void {
  ...
  const byStatus: Record<OpenLoopStatus, number> = {
    todo: 0,
    doing: 0,
    done: 0,
    snoozed: 0,
  ...
  const highSeverityLoops = loops.filter((l) =>
    HIGH_SEVERITIES.includes(l.severity) && l.status !== 'done'
  );

  const activeCandidates = loops.filter(
    (l) => (l.status === 'doing' || l.status === 'todo') && HIGH_SEVERITIES.includes(l.severity)
  );
