...
const DEFAULT_CONFIG: StuckLoopPolicyConfig = {
  todoTimeoutHours: 48,
...
function isOverdue(loop: OpenLoopState, nowIso: string): boolean {
  if (!loop.dueAt) return false;
  return new Date(loop.dueAt).getTime() < new Date(nowIso).getTime();
}

function unchanged(loop: OpenLoopState): StuckLoopEvaluation {
  return {
...
export function evaluateStuckLoop(
  loop: OpenLoopState,
  nowIso: string = new Date().toISOString(),
  config: StuckLoopPolicyConfig = DEFAULT_CONFIG
): StuckLoopEvaluation {
  if (
    (loop.severity === 'high' || loop.severity === 'critical') &&
...
export function getDefaultStuckLoopPolicyConfig(): StuckLoopPolicyConfig {
